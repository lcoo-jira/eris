...
class ErisFuelInvTest(base.TestCase):

    def test_erisfuelinv(self):

        # Read in the config file into json
        with open('eris/tests/datafiles/test_fuel_config.json','r') as fid:
            eris_config = json.load(fid)
        
        # Remove the inventory plugin
        if 'inventory_plugin' in eris_config:
            eris_config.pop('inventory_plugin')

        inv_obj = fuelinv.ErisAnsibleInventory(eris_config)
